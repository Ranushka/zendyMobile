...
class LoadWebScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    //   var _pdfView = _pdfInst.cachedFromUrl(
    //     _downloadLink,
    //     headers: _headers,
    //     placeholder: (progress) => _buildProgressIndicator(progress),
    //     errorWidget: (dynamic error) => _buildPdfError(error),
    //   );
    //   bodyContent = _pdfView;
    // } else if (_readLink.isNotEmpty) {
    var _webView = WebView(
      javascriptMode: JavascriptMode.unrestricted,
      initialUrl: _downloadLink,
      onWebViewCreated: (controller) {
        Map<String, String> header = {
          'Cookie': authCtrl.currentUser.value.authToken
        };

        controller.loadUrl(_downloadLink, headers: header);
